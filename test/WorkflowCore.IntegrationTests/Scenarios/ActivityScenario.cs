...
public class ActivityScenario : WorkflowTest<ActivityScenario.ActivityWorkflow, ActivityScenario.MyDataClass>
{
    ...
    public void Scenario()
    {
        ...
        var activity = Host.GetPendingActivityAsync("act-1", "worker1", TimeSpan.FromSeconds(30)).Result;

        if (activity != null)
        {
            var actInput = (ActivityInput)activity.Parameters;
            Host.SubmitActivitySuccessAsync(activity.Token, new ActivityOutput
            { 
