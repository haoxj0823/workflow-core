...
public class DefinitionLoader : IDefinitionLoader
{
    ...
    private WorkflowStepCollection ConvertSteps(ICollection<StepSourceV1> source, Type dataType)
    {
        ...
        var compensatableSteps = new List<StepSourceV1>();
        ...
        while (stack.Count > 0)
        {
            ...
            if (stepType.GetInterfaces().Contains(typeof(IStepBody)))
            {
                ...
                targetStep = containerType.GetConstructor([]).Invoke(null) as WorkflowStep;
            }
            else
            {
                targetStep = stepType.GetConstructor([]).Invoke(null) as WorkflowStep;
                if (targetStep != null)
                {
            ...
            if (!string.IsNullOrEmpty(nextStep.CancelCondition))
            {
                ...
                var cancelExpr = DynamicExpressionParser.ParseLambda<bool>(ParsingConfig, false, [dataParameter], TransformExpression(nextStep.CancelCondition));
            ...
            if (nextStep.CompensateWith != null)
            {
                ...
                {
                    compensatableSteps.Add(nextStep);
                }
            }
        ...
        foreach (var item in compensatableSteps)
        {
    ...
    private void AttachOutputs(StepSourceV1 source, Type dataType, Type stepType, WorkflowStep step)
    {
        foreach (var output in source.Outputs)
        {
            ...
            var sourceExpr = DynamicExpressionParser.ParseLambda<object>(ParsingConfig, false, [stepParameter], TransformExpression(output.Value));
            var dataParameter = Expression.Parameter(dataType, "data");
    ...
    private void AttachDirectlyOutput(KeyValuePair<string, string> output, WorkflowStep step, Type dataType, LambdaExpression sourceExpr, ParameterExpression dataParameter)
    {
        ...
        else
        {
            ...
            void acn(IStepBody pStep, object pData)
            {
                object resolvedValue = sourceExpr.Compile().DynamicInvoke(pStep); ;
                propertyInfo.SetValue(pData, resolvedValue, [output.Key]);
            }
        ...
        foreach (var propertyName in paths)
        {
            ...
            if (propertyName.Contains("["))
            {
                ...
                void acn(IStepBody pStep, object pData)
                {
                    ...
                    object resolvedValue = sourceExpr.Compile().DynamicInvoke(pStep); ;
                    propertyInfo.SetValue(data, resolvedValue, [items[1]]);
                }
    ...
    private void AttachOutcomes(StepSourceV1 source, Type dataType, WorkflowStep step)
    {
        ...
        {
            var sourceDelegate = DynamicExpressionParser.ParseLambda<object>(ParsingConfig, false, [dataParameter, outcomeParameter], TransformExpression(nextStep.Value)).Compile();
    ...
    private static Action<IStepBody, object, IStepExecutionContext> BuildScalarInputAction(KeyValuePair<string, object> input, ParameterExpression dataParameter, ParameterExpression contextParameter, ParameterExpression environmentVarsParameter, PropertyInfo stepProperty)
    {
        ...
        var sourceExpr = DynamicExpressionParser.ParseLambda<object>(ParsingConfig, false, [dataParameter, contextParameter, environmentVarsParameter], TransformExpression(expr));
        ...
        void acn(IStepBody pStep, object pData, IStepExecutionContext pContext)
        {
            ...
            while (stack.Count > 0)
            {
                ...
                foreach (var prop in subObj.Properties().ToList())
                {
                    ...
                    {
                        var sourceExpr = DynamicExpressionParser.ParseLambda<object>(ParsingConfig, false, [dataParameter, contextParameter, environmentVarsParameter], TransformExpression(prop.Value.ToString()));
                        var resolvedValue = sourceExpr.Compile().DynamicInvoke(pData, pContext, Environment.GetEnvironmentVariables());
